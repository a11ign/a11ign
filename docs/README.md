...
| [outsider-runs.md](./outsider-runs.md) | you are recording what someone outside the project said about a run of the tool, or want to see the fixed fields a record must have and the first outsider's, with `NOT STATED` where they said nothing |
...
| [milestone-triage.md](./milestone-triage.md) | **every open row in "Road to version one", sorted by `ceo`'s on-path test** (#2273): *a row is on the path iff it stops an outsider running the tool on their product or trusting the result.* A reading at a moment (2026-09-25), from row bodies. Says which rows moved to `Out of release`, which are OFF but held because a session has them in progress, and why a `fleet-gated` row filed off-path is not offered to `orchestrator` |
...
| [mutant-replay.md](./mutant-replay.md) | **would a machine-chosen set of mutants have found what the reviewer found?** — a generator replayed over the three refusals it was meant to catch (#2384, #2368, #2392) at the commits refused. **NULL RESULT (`ceo`, #2415): 1 exact and in-sample, 1 half, 1 miss on reach; nothing ships into `pr:open`**, and the record says what the operators missed and why |
