...
| consumer-gate.yml | yes (2) | YES, the published Action pinned by commit SHA at line 51 (a reference to this repository) | code-only (the published Action) |
...
**Sized, and it is not the file count.** The ten row bodies declare the Region paths below (item 1's directory
entry counts once and stands for its 61 files), against the tracked files of the two packages that move:

```
$ node -e 'const app=require("fs").readFileSync("docs/adr/0039-the-split-is-mostly-org-machinery.md","utf8").split("\n## Appendix")[1];const all=new Set();let lines=0;for(const m of app.matchAll(/^## Region\n+```\n([\s\S]*?)\n```/gm))for(const l of m[1].split("\n").filter(Boolean)){lines++;all.add(l.replace(/ \(new\)$/,""))}console.log("region lines",lines,"distinct paths",all.size)'
region lines 62 distinct paths 60
```

```
$ git ls-files packages/nvda-worker packages/nvda-speech | wc -l
135
```

By files the machinery is comparable to the code, not larger: 60 Region paths plus the 61 files behind item 1's one directory entry is about 120, against 135. **What makes it the
...
```
$ node -e 'import("./packages/agent-org/src/row-file.mjs").then(m=>{const app=require("fs").readFileSync("docs/adr/0039-the-split-is-mostly-org-machinery.md","utf8").split("\n## Appendix")[1];for(const [,n,b] of app.matchAll(/^### ROW (\d+)\n\n````markdown\n([\s\S]*?)\n````/gm))console.log("ROW",n,"region:",m.regionRefusalReason(b)??"ok","acceptance-shape:",m.acceptanceShapeRefusal(b)??"ok","filing:",(m.fileRefusalReason(b)??"ok").slice(0,45))})'
