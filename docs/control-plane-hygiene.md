...
| **`/tmp` user quota** | RULE: read the QUOTA, never `df` (#2220). `/tmp` on the agent host is a tmpfs mounted `usrquota`, and a user can be refused a write while the filesystem has room: `df` read 80% and 3.1 GB free while a 50 MB write failed with `Disk quota exceeded` (2026-09-23), and the only symptom was unrelated test files that build git sandboxes under `/tmp` going red. The report reads the quota itself (`quotactl_fd`, no `quota` binary is installed here) and prints **EXHAUSTED** (at or over the limit), **CONSTRAINED** (90% or more), **OK**, or **NOT MEASURABLE** with its reason: the mount declares no `usrquota`, no limit is set for the user, or a part could not be read. **NOT MEASURABLE is not a pass** — no quota and a fine quota are different answers. The verdict never consults `df`, which is printed only as the contrast. What the quota is spent on is 1,000-odd per-session scratchpads under `/tmp/claude-<uid>` that nothing removes (measured in #2220); clearing them is a HOST act that must tell a live scratchpad from an abandoned one, so this report only counts and never deletes. |
