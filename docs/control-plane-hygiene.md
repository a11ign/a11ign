...
| **`/tmp` user quota** | RULE: read the QUOTA, never `df` (#2220). **The incident this was built for is history on this host:** on 2026-09-23 `/tmp` was a tmpfs mounted `usrquota`, and a user could be refused a write while the filesystem had room: `df` read 80% and 3.1 GB free while a 50 MB write failed with `Disk quota exceeded`, and the only symptom was unrelated test files that build git sandboxes under `/tmp` going red. Since then `/tmp` has moved onto `/` (ext4, no `usrquota`), and the 2026-09-25 outage was inode exhaustion, not a quota (the free-space and free-inode watch is #2163). So on the current host the row reads **NOT MEASURABLE** ("the mount declares no usrquota"), which is the intended answer there; the reader stays for a host that has a quota. The report reads the quota itself (`quotactl_fd`, no `quota` binary is installed here) and prints **EXHAUSTED** (at or over the limit), **CONSTRAINED** (90% or more), **OK**, or **NOT MEASURABLE** with its reason: the mount declares no `usrquota`, no limit is set for the user, or a part could not be read. **NOT MEASURABLE is not a pass** — no quota and a fine quota are different answers. The verdict never consults `df`, which is printed only as the contrast. In the 2026-09-23 incident the quota was spent on 1,000-odd per-session scratchpads under `/tmp/claude-<uid>` that nothing removes (measured in #2220); clearing such things is a HOST act that must tell a live scratchpad from an abandoned one, so this report only counts and never deletes. |
