...
**EVERY WAKE LOADS THIS FILE, AND IT IS BUDGETED AT 20,000 BYTES WITH `CLAUDE.md` (#2217).** It carries
the RULE and one clause of why; **the incident behind each is in
...
`packages/lab/src/packaging/prefix-budget.test.ts` prints what it costs. **Evict or move, never truncate.**
...
- **Do not create a cron to check for work.** If you want one, the gate is missing a question — add it to
  ...
  event (a nightly, a board edition), never for "has anything changed yet".

- **The row is the state.** Read the row, the PR and the API before acting on any message, `ceo`'s included.
...
- **Pools are per TOKEN and per RESOURCE, and the endpoint can be right about one while lying about the
  ...
  accurate on `core` to within one call, wrong by 1,360 on `graphql`. `gh pr view`/`pr list`/`issue list`
  spend GRAPHQL; `gh api` spends CORE. Read the pool you are about to spend.
...
- **A `lane:<owner>` label refuses any OTHER session unconditionally** (`laneReason`,
  ...
  does. If the owner is the only session that can claim a row, its turn budget is the queue's throughput.
- **The test before leaving a row in a lane other than `any`: does the label protect a DECISION only the
  ...
  their hands?** A path re-lanes to `lane:any` and the owner still reviews; a decision stays, and the
  wait is then a real cost of it. Full ruling: `docs/lane-ownership.json`'s `_claimVsAuthorRuling`.

## `main` REQUIRES an approving review (ceo, 2026-09-22, #2022)


- **One approving review, and `bypass_pull_request_allowances` EMPTY.** The allowance goes with it or the
  requirement is decorative — its sole entry queued a large share of merges, so the rule would read as
  universal while absent on half of them.
- **An `a11ign-bot`-authored PR waits.** The hatch is a human admin editing the protection;
...
- **Read it back BEHAVIOURALLY, never off the field** (`branch-protection.test.ts`):
  ...
  **`reviewDecision`**, EMPTY when the base requires no approval — and it needs no admin, which matters
  because `a11ign-ai-workers` has `permissions.admin: false`.
- **A 404 from `branches/main/protection` means absent OR forbidden, never "unprotected".**
  `branches/main.protected` is the discriminator and needs no admin. A verdict that cannot read
  `bypass_pull_request_allowances` is `CANNOT_TELL`, loudly — never a pass.
...
- **`main` KEEPS stale reviews on BOTH surfaces; the defect is closed by READING `reviewDecision`, not by
  dismissing reviews.** `dismiss_stale_reviews` covers APPROVING reviews only, so it could not have cleared
  the `CHANGES_REQUESTED` that stalled #2049 for seven hours — and *Update branch*, which
  `update-branch-sweep.mjs` runs after every merge, would have stalled 15 of the last 40 merges.
- `readPrs` asks for `reviewDecision` on the `pr list` call it already makes — another field, not another
  call — and **`pr-review-blocked`** names every green, unheld PR GitHub is holding, including one that
  opened READY and so never entered the reviewer lane.
- **BOTH SURFACES ARE READ, because naming one is not enough.** `dismiss_stale_reviews` (classic) and
  `dismiss_stale_reviews_on_push` (ruleset) are independent; `branch-protection.test.ts` reads both and
  refuses a DISAGREEMENT. The classic surface needs repo admin and is `CANNOT_TELL` everywhere here.
...
- **Waiting on a SESSION to answer → label the row `answer:<session>`. Removing the label IS the act of
  answering**, so there is nothing to remember. A label and not an assignee: eight sessions share four
  accounts.
...
- **All of these CLEAR THEMSELVES, and that is the point.** `blocked` is a claim with **no referent** — it
  never says what blocks the row, so only a human re-reading it can lift it. Use it only for a wait no
  field can express, and say what would clear it.
...
- **`product-manager` is the first reader, and ruler, for rows, the queue and process:** filing and
  ...
  close-outs and host-run announcements. An engineer's report goes there, never to `ceo`.
...
- **That reading is POSTED and DELIVERED, and those are two jobs (#2083).** Post on **#928** (the RECORD),
  then deliver it with **`npm run prompt:session -- ceo "…"`** — posting alone reaches nobody. **Both
  halves, or it is unrecorded or undelivered.** "One per `ceo` tick" is a RATE, not a clock.
...
- **The author of a draft prompts its parity reviewer** on open and after every head-changing push:
  ...
  CLEARS THE SESSION FIRST and a raw `herdr … agent prompt` does not** — use the raw call only to RE-prompt
  the same draft. No verdict 30 minutes after it goes to `product-manager`, who re-prompts once, then `ceo`.
- **ONE CALL IS ENOUGH, AND RETRYING IS THE WRONG THING (#1966).** `prompt:session` **queues** the order
  ...
  and do not poll:** it clears its target first, so a retry landing as a session goes idle wipes the work it
  interrupted. A refusal naming a session the org does not know is a typo — NOT queued, and it says so.
...
- **Write `rm -f "${D:?}"/*.md`, never `rm -f $D/*.md`.** `:?` makes the shell abort on an unset or empty
  ...
  `--dangerously-skip-permissions` deliberately does not disable, so it reaches a human every time.
- **The cost is not the seconds.** The chairman approved one such line several times in one morning, each
  time reading a command in order to conclude it was fine. Every avoidable prompt spends a human's
  attention and **makes the unavoidable ones cheaper to ignore.**
...
- **Prevention rather than cleanup.** Quoting alone defuses the BARE-VARIABLE case, and buys nothing once a
  glob is attached: an empty `"$D"` in `rm -f "$D"/*.md` still expands to `rm -f /*.md`, because the glob
...
- **Count this population with `roles-readme.test.ts`'s classifier, which splits the arguments and drops
  ...
  expects the target, so a line-anchored grep under-reads the population it is meant to check.
