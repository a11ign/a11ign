...
function howToReadThisSection(): string[] {
  return [
    ...
    "Its number is a cosine similarity to the closest page the scorer trained on, from -1 (as unlike that",
    "page as the space allows) to 1 (identical); the scorer's own training pages sit at roughly 0.85-0.99",
    "from their nearest neighbour, so a \"floor\" a little under that band is the cutoff below which a page",
    "no longer resembles anything the scorer was shown.",
