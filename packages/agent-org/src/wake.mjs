...
import { JUDGMENT_CAUSES, CHAIRMAN_LABEL, LAUNCH_PLACEHOLDER } from "./work-gate.mjs";
 ...
 * answers `{"type":"ok"}`.
 *
 * THE PANE STARTS IN THE ROW'S WORKTREE WHEN THERE IS ONE (#2405). Without `--cwd` herdr's default is
 ...
 * peer's tree. A `cwd` of `undefined` is the old default, kept for a caller that has claimed nothing.
 *
 ...
 * @param {LaunchFacts & {spawned?: ClaimedRow}} [facts]
 */
export function addressed(order, label, { spawned, ...launch } = {}) {
...
export function deliver(orders, agents, roster,
  ...
  for (const order of orders) {
    ...
    const target = targetFor(order, live, roster,
      { run, spawned, ineligibleReason, env, registerSpawn, drained, claimable, claimer });
    ...
    try {
      run(["--session", "org", "agent", "prompt", target.label,
        addressed(order, target.label, { ...launch, spawned: target.claimed })]);
    } catch (err) {
 ...
 *
 * @param {{ exec?: Exec, exists?: (path: string) => boolean, worktreesDir?: string, primary?: string }} [host]
 *   every one a seam, so the claim is testable without a host: the defaults are the tick's own
