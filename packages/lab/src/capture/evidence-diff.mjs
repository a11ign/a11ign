 ...
 * and not by a length: no threshold is taken on trust, because the shortest LEGITIMATE transcript is not known.
 * The only corpus copies readable when this was written held two Southwark captures each (240 and 250 lines
 * admitted), which bounds nothing, so a "few lines" rule would be a number with no source.
 *
