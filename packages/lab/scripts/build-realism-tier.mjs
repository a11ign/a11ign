 ...
 *
 * So are the captures rejected as truncated (#2383): the count alone (`4 of 41`) could not say which pages
 * changed sides between two builds. `[]` on the base-only path -- an absent key and an empty list differ.
 */
function writeProvenance(/** @type {any} */ baseText, /** @type {any} */ records, /** @type {any} */ census, /** @type {any} */ rejected) {
  ...
  writeFileSync(path, JSON.stringify({
    ...
    rejectedAsTruncated: rejectedAsTruncated(rejected),
  }, null, 2) + "\n");
  return path;
}
...
function main() {
  ...
  const provenance = writeProvenance(baseText, records, census, rejected);
